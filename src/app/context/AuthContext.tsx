...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const [user, setUser] = useState<{
    ...
    email: string;
    locale?: string;
  } | null>(null); // <--- تم إضافة locale
  const [isLoadingAuth, setIsLoadingAuth] = useState<boolean>(true); // عند تحميل التطبيق، تحقق من وجود التوكن وبيانات المستخدم في localStorage
  ...
  useEffect(() => {
    ...
    const storedUserLocale =
      typeof window !== "undefined" ? localStorage.getItem("userLocale") : null; // <--- جلب Locale
  ...
  return (
    <AuthContext.Provider
      value={{ isAuthenticated, user, login, logout, isLoadingAuth }}
    >
      {children}
    </AuthContext.Provider>
  );
