...
export interface CartItem {
  // <--- تم إضافة export لتكون متاحة للاستخدام في ملفات أخرى
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  const [cartItems, setCartItems] = useState<CartItem[]>([]); // تحميل السلة من Local Storage عند تحميل المكون
  ...
  useEffect(() => {
    ...
    if (storedCart) {
      try {
        ...
        console.error(
          "CartContext: Error parsing stored cart from localStorage",
          error
        );
  ...
  useEffect(() => {
    console.log(
      "CartContext: cartItems changed, attempting to save to localStorage:",
      cartItems
    );
  ...
  }, [cartItems]); // إضافة عنصر إلى السلة أو تحديث كميته إذا كان موجودًا
  ...
  const addItem = (item: CartItem) => {
    ...
    setCartItems((prevItems) => {
      ...
      if (existingItemIndex > -1) {
        ...
        const existingItem = updatedItems[existingItemIndex]; // ⭐ تم التعديل هنا: بدلاً من الإضافة، نقوم بتعيين الكمية الممررة مباشرةً
        ...
        console.log(
          "CartContext: Item updated in cart:",
          updatedItems[existingItemIndex]
        );
        return updatedItems;
      } else {
  ...
  const updateItemQuantity = (id: string, quantity: number) => {
    console.log(
      `CartContext: Attempting to update item ID: ${id} to quantity: ${quantity}`
    );
    setCartItems((prevItems) => {
      const updatedItems = prevItems
        .map((item) =>
