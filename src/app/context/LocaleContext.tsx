...
  currentLocale: string; // مثال: 'ar-SY' للأرقام الهندية، 'en-US' للأرقام العربية الغربية
  // تم تحديث نوع setLocale ليقبل إما سلسلة نصية مباشرة أو دالة تحديث الحالة
...
  // وظيفة لضبط اللغة بشكل صريح
  // هذه الدالة ستستدعي setCurrentLocale مباشرة، والتي تستقبل أيضاً دالة تحديث الحالة
...
        setCurrentLocale(newLocale);
    }
    // هنا، إذا كنت تريد حفظ تفضيل اللغة في الـ Backend:
    // يمكنك استدعاء API لإنقاذ تفضيل اللغة الجديد للمستخدم
    // مثلاً: updateUserLocaleOnBackend(newLocale);
  };
  
...
    return amount.toLocaleString(currentLocale, { style: 'currency', currency: 'USD' });
  };

  return (
    <LocaleContext.Provider value={{ currentLocale, setLocale, toggleLocale, formatCurrency }}>
      {children}
