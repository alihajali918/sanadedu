...
interface Donation {
  ...
  caseName: string;
  amount: number;
  status: DonationStatus | string; // بعض الـ APIs قد تعيد نصوصًا مختلفة
  date: string; // ISO
  currency: string;
}

const DonationsPage: React.FC = () => {
  const { status: authStatus } = useSession(); // نقرأ الجلسة من NextAuth (لا نحتاج JWT هنا)
    ...
    (async () => {
      try {
        ...
        const res = await fetch("/api/donations", {
          method: "GET",
          cache: "no-store",
          credentials: "include", // مرّر كوكي الجلسة فقط
          signal: ac.signal,
        });
        ...
        if (data?.ok && Array.isArray(data.donations)) {
          setDonations(
            data.donations.map((d: any) => ({
              // تم التعديل هنا: استخدام `d.totalAmount` بدلاً من `d.amount`
              id: String(d.caseId ?? d.id),
              caseId: String(d.caseId ?? "غير معروف"),
              caseName: String(d.caseName ?? "—"),
              amount: Number(d.totalAmount ?? 0),
              status: String(d.status ?? "مكتمل"), // بما أننا نجمع، نفترض أنها مكتملة
